...
function targetless.Buffer:new()
    ...
    function buffer:addship(id)
      local ship = targetless.Ship:new(id)
        ...
        local guard = false
        if ship.ship == "Guard" or ship.ship == "Marshal" then
        guard = true
        end
        local stturret = false
        if ship.ship == "Turret" then
        stturret = true
        end
        if ship.cap then self.count.cap = self.count.cap + 1 end
        if bomb then self.count.bomb = self.count.bomb + 1 end
        if guard then self.count.guard = self.count.guard + 1 end
        if stturret then self.count.stturret = self.count.stturret + 1 end
        self.count.all = self.count.all + 1
	    ...
	    if not guard and ship.npc then return end
        elseif self.mode == "StTurret" then
	    if ship.ship ~= "Turret" and ship.npc then return end
        end
        self.ships:add(ship)
    end
