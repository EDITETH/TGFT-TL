...
function targetless.Ship:new(id)
    ...
    if status == 3 then ship.hostile = false
		...
		--ship.hostile = true  --Make all ships hostile
		if targetless.Controller.mode == "StTurret" then 
			--print("Turrets") --Lisa
			ship.hostile = true  --Make all ships hostile
		end
    ...
    if string.find(ship.name, "Station Guard") and ship.npc then
			ship.ship = "Station Guard"
    elseif string.find(ship.name, "Marshal") and ship.npc then
      ship.ship = "Marshal"
    end
    ...
    function ship:getlabelbytag(tag)
	    ...
	    if ship == "Turret" then ship = string.sub(self.name,string.find(self.name,'Turret'),#self.name) end
            iuplabel.title = ship
	...
	elseif(tag == "<autox>") then
	    if targetless.Controller.autox == 'autoxactive' then
	        iuplabel = iup.label {title = "AUTOX", font = self.font * 1.25, fgcolor = self.fontcolor }
	    elseif targetless.Controller.autox == 'autoxoff' then
	        iuplabel = iup.label {title = "", font = self.font, fgcolor = self.fontcolor }
	    end
	elseif(tag == "<dsort>") then
	    if targetless.List.dsort == 'descending' then
		iuplabel = iup.label {title = "DSORT", font = self.font * 1.25, fgcolor = self.fontcolor }
	    elseif targetless.List.dsort == 'ascending' then
		iuplabel = iup.label {title = "", font = self.font, fgcolor = self.fontcolor }
	    end
        end
